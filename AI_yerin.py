...
WEIGHTS = {
    ...
    "formation": 0.104,
    "mobility": 0.109,
    "threat_assessment": 0.081,
    "self_trap_risk": 0.305
}
...
def is_in_array(coord, arr):
    if len(arr) > 10:
        ...
        return tuple(coord) in arr_set

    for i in range(len(arr)):
        if arr[i][0] == coord[0] and arr[i][1] == coord[1]:
            return True
    return False
...
def evaluate_threat_assessment(friend_positions, enemy_set):
    safety_score = 0.0
    max_positions = min(len(friend_positions), 10)
    positions_to_check = friend_positions[: max_positions]
    for pos in positions_to_check:
        pos_tuple = tuple(pos)
        if pos_tuple in EDGE_DIRECTIONS:
            for nbr in get_neighbors(pos):
                if nbr in enemy_set:
                    safety_score -= 0.4
    return safety_score
...
def evaluate_self_trap_risk(opponent_positions, friend_set):
    safety_score = 0.0
    max_positions = min(len(opponent_positions), 10)
    positions_to_check = opponent_positions[:max_positions]
    for pos in positions_to_check:
        safety_score -= 0.2
        for nbr in get_neighbors(pos):
            if not nbr in friend_set:
                safety_score += 0.2
                break
    return safety_score
...
def evaluate_board(board, player):
    ...
    threat_score = (
            evaluate_threat_assessment(friend_positions, enemy_set)
            - evaluate_threat_assessment(enemy_positions, friend_set)
    )

    enemy_player = "white" if player.lower() == "black" else "black"

    self_trap_score = (
            evaluate_self_trap_risk(enemy_positions, friend_set)
            - evaluate_self_trap_risk(friend_positions, enemy_set)
    )
    ...
    features = {
        ...
        # "blocking_pwr": blocking_score,
        "self_trap_risk": self_trap_score
    }
