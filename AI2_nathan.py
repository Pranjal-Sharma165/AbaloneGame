...
def evaluate_move_restriction(enemy_moves):
    # enemy_idx = 1 if player.lower() == "black" else 0
    # enemy_color = "WHITE" if player.lower() == "black" else "BLACK"
    # enemy_moves = move_generator(board, enemy_color)

    if not enemy_moves:
        return 100.0

    return -len(enemy_moves) * 1.5
...
#total score is in this function
#changing from 3 to 2.5 sec
def evaluate_board(board, player, opp_moves_dict):
    ...
    # move_restriction_score = evaluate_move_restriction(board, player, opp_moves_dict)
    move_restriction_score = evaluate_move_restriction(opp_moves_dict)
    ...
    features = {
        ...
        'move_restriction': move_restriction_score
    }

    weights = WEIGHTS.copy()
    weights['move_restriction'] = 0.4
...
def alpha_beta_with_time_check(board, depth, alpha, beta, player, maximizing_player,
    ...
    if depth <= 0:
        eval_score = evaluate_board(board, maximizing_player, moves_dict)
