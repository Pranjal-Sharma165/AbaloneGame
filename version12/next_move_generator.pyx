...
# Import optimized functions from move
...
def read_board_from_text(filename):
    """
    Read board state from a text file.
    """
...
def check_marbles(marble_tuple):
    """
    Check if a group of marbles forms a valid group according to game rules.
    """
...
def find_all_groups_of_size_1_2_3(board, color):
    """
    Find all valid groups of 1, 2, or 3 marbles for a given color.
    """
...
def generate_all_directions(marble_list):
    """
    Generate potential moves in all six directions for a marble group.
    """
    result = {}
...
def generate_all_next_moves(board, color):
    """
    Generate all possible next moves for a given board state and color.
    """
    marble_groups = find_all_groups_of_size_1_2_3(board, color)

    # Dictionary to store results in the requested format
    result_dict = {}

    # Process each possible source group
    for source in marble_groups:
        # Generate all possible movement directions for this source group
        directions = generate_all_directions(source)

        # Check each direction for valid moves
        for direction_name, dest in directions.items():
            # Skip if any destination is off the board
            ...
            if is_valid:
                ...
                if new_board is not None:
                    # Convert source and dest lists to tuples of tuples for dictionary key
...
def save_board_states_to_file(board_states, filename, next_player_color):
    """
    Save generated board states to a file for analysis.
    """
